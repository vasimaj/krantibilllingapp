...
import { saveDeptMasterApi, updateDeptMasterApi, getDeptMasterApi , deleteDeptMasterApi} from '../../Services/MasterServices/deptMasterAPIURL';
...
const DepartmentMaster = () => {
    ...
    const columns = [
        { field: 'department_id', headerName: 'ID', width: 80, headerClassName: 'mc-table-head primary' },
        {
            field: 'department',
        ...
        },
        {
            ...
            width: 100,
            renderCell: (params) => {
                const onClick = (department_id, department) => {
                    setName(department);
                    setId(department_id);
                };
                ...
                return <>
                    <span onClick={() => {
                        onClick(params.row.department_id, params.row.department);
                    }}
                    ...
                    <span onClick={() => { setAlertModal(true); setId(params.row.department_id) }}
    ...
    const getTypeMasterData = async () => {
        await axios.get(getDeptMasterApi)
            .then((res) => {
                if (res.data === "ERROR") {
    ...
    const updateFun = async () => {
        ...
        await axios.post(updateDeptMasterApi, updateObj)
            .then((res) => {
                if (res.data === "UPDATED") {
                    clearFun();
                    getTypeMasterData();
                    toast.success("Department Master updated sucessfully");
                }
                else {
    ...
    const insertFun = async () => {
        ...
        await axios.post(saveDeptMasterApi, saveObj)
            .then((res) => {
                if (res.data === "SAVED") {
                    getTypeMasterData();
                    clearFun();
                    toast.success("Department Master saved sucessfully");
                }
                else {
                    toast.error("Operation Failed.");
                }
    ...
    const delFun =async () => {
        ...
        await axios.post(deleteDeptMasterApi, delObj)
            .then((res) => {
                if (res.data === "DELETED") {
                    
                    getTypeMasterData();
                    toast.success("Department Master deleted sucessfully");
                }
                else {
                    toast.error("Operation Failed.");
                }
            ...
            <Row>
                <Col xl={12}>
                    <Box className="mc-card">
                        <Breadcrumb title={"Department Master"}>
                ...
                <Col xl={4}>
                    <Form onSubmit={saveFun}>
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            ...
                            <Form.Control type="text" placeholder="Enter department..." required
                                value={name}
